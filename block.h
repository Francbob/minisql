...
public:
	bool isValid(int record_index) const {
		return record_index<size && !bitmap[record_index];
	}
	string get_attr(int record_index, int attr_index) const;
	void show_record(int record_index) const;
	bool delete_record(int record_index) {
		if (!isValid(record_index))
			return false;
		bitmap[record_index] = true;
		return true;
	}
