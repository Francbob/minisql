...
//Operations of tables
	...
	void API_SelectAll(string Table_name);
	...
	void API_Delete(string Table_name, vector<condition> delete_condition);
...
private:
	...
	//Return 0: ALL attributes in condition have index.
	//Return 1: There's no index in all attriutes
	//Return 2: Some attributes with index
	int API_IfAllIndex(string Table_name, vector<condition> select_condition, vector<condition> &Condition_with_index, vector<condition> &Condition_no_index);	
